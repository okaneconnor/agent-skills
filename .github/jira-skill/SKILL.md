...
```bash
python3 .github/jira-skill/scripts/post-to-jira.py /tmp/draft_<name>.md --dry-run
```
...
```bash
python3 .github/jira-skill/scripts/post-to-jira.py /tmp/draft_<name>.md --epic <EPIC-KEY>
```

The script reads credentials from `.github/jira-skill/scripts/.env` (copy from `.env.example`). Requires `pip install requests`.
